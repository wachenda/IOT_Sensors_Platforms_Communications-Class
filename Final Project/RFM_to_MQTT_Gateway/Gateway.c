...
#define MQTT_CLIENT_ID "0123456789ABCDEF"  // Need to change for your setup, Must be exactly 16 characters in length and match other nodes
...
int main(int argc, char* argv[]) {
	...
	if( mosquitto_username_pw_set(m,"openhabian","WoJ3w3vx3tEiLGdG[vxE") != MOSQ_ERR_SUCCESS)
		{ die("MQTT Authentication Failure\n"); }

	//if(mosquitto_tls_set(m, "ca.crt", NULL, NULL,NULL, NULL) != MOSQ_ERR_SUCCESS)
	//{ die("MQTT TLS ca.crt failure\n");}
	//
	/*int ret;
	char cafile[]   = "/etc/mosquitto/ca_certificates/ca.cert";
	char certfile[] = "/etc/mosquitto/certs/openHABianPi.crt";
	char keyfile[] = "/etc/mosquitto/certs/openHABianPi.key";
	 ret = mosquitto_tls_set(m, cafile, NULL, certfile, keyfile, NULL);
